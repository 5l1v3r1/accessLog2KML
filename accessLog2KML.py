...
def _add_ip(ip):
    ip_json = json.loads(urlopen("https://ipinfo.io/" +
                                 ip + "/json").read().decode())
    ...
    print("<Placemark>\n<name>" + ip + "</name>\n<description>Country:" + country + "<br />City:" +
          ip_json['city'] + '<br />Region:' + ip_json['region'] + "<br /></description>\n<Point>\n<coordinates>" + coords[1] + ',' + coords[0] + "</coordinates>\n</Point>\n</Placemark>\n")
...
logins_by_country = OrderedDict(
    sorted(logins_by_country.items(), key=itemgetter(1), reverse=True))
logins_by_country = json.dumps(logins_by_country, indent=4)
logins_by_asn = OrderedDict(
    sorted(logins_by_asn.items(), key=itemgetter(1), reverse=True))
